...
time by `social-os:update-the-brain` — because a contradiction is born the instant a
...
assumes it. Four checks, all from `0 — Map` plus one folder listing:
   ...
   - **Two documents, one role.** List `1 — Brain/Told to us/`. Does anything hold a
     ...
     yourself: hand it to `social-os:update-the-brain`, which quotes both and asks
     her which is right.
...
`0 — Map`. If either is missing, stop and run `social-os:brand-onboarding`
