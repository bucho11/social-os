...
`0 — What's Installed`). Three index documents can disagree about which folder the
...
with `social-os:update-the-brain`, or run `social-os:brand-onboarding` if there is no
workspace at all.
