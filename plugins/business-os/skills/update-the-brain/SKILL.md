...
Read `${CLAUDE_PLUGIN_ROOT}/shared/the-law.md` — the six laws — and
`${CLAUDE_PLUGIN_ROOT}/shared/the-map.md` before doing anything here. The six laws
in one line each:
...
4. **A change and a correction are different.** One leaves old work honest; the
   other makes it false.
5. **Every change has a blast radius**, handled in the same turn.
6. **Nothing is destroyed.** Superseded and archived, never trashed.
...
Found one → that is the bug, and it outranks whatever she asked about.

**Reconcile them point by point. Do not ask her to pick one.** Each document will
hold something the other doesn't — that is why both got written. Choosing one whole
throws away real work she did, and she will notice in about a month when the thing
she wrote has quietly stopped happening.

So: read both. Where only one says something, keep it. Where they genuinely
disagree, that is a question — and usually there are only one or two of those:

> You have two documents about how you sound, and they mostly agree. They differ in
> one place: **Brand voice** says no emoji, **Tone notes** says a couple is fine.
> Which is right?

Then write one reconciled document, **supersede the other** (Law 6 — renamed with
the date, moved to `9 — Archive/`, never trashed), and say in one line what you
kept from each.
...
### 3b · Ask which kind of change this is

Only when a **fact** is changing — a price, a service, a credential, a claim — and
it isn't already obvious:

> Did that change, or was it always wrong?

Three words back, and it decides what happens to everything already published
(Law 4). *"We charge $30 now"* means the posts saying $25 were correct when they
went out — leave them alone. *"We've never charged $25"* means those posts are
false and she needs to know today.

Skip this for style and preference changes. A voice correction has no truth value
to be retroactively wrong about.
...
"previously" section. Rules hold only what is true now.

**Create the new version, then supersede the old one** — rename it with today's date
and move it to `9 — Archive/`. Never `trash_file`: Google deletes trash after 30
days, and the record of what her brand voice used to be is exactly what you want on
the day a correction turns out to have been wrong. Same number of calls.
`${CLAUDE_PLUGIN_ROOT}/shared/drive-conventions.md` has it.
...
- Never resolve two documents by picking one whole. Reconcile point by point, ask
  only about the places they actually disagree, supersede the loser.
- Never trash a rule document. Supersede it — renamed, dated, archived, readable.
...
`business-os:brand-onboarding`. Folder there but no `0 — Map` → rebuild the map from
