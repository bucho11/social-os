...
description: >
  Build a business owner's AI workspace from nothing, in one warm conversation:
  research their website and Instagram first, interview them one question at a time
  about the business, its two audiences, brand voice, offers, rules and how they want
  to be worked with, then create their Google Drive workspace and every document in
  it, set up their publishing accounts, and draft three real posts so they see it
  working the same day. Use when an owner is new or says "set me up", "get started",
  "onboard me", "connect my accounts", "you don't know my brand yet", or asks what
  this can do — and when any other skill finds no workspace in Drive at all. Use it
  again only to re-run a whole section of the interview from scratch. For changing
  something that already exists, use update-the-brain instead.
---
...
**Where this skill stops and `update-the-brain` starts.** This one **builds** — it
creates documents that do not exist yet, by interviewing her. `update-the-brain`
**changes** — it edits a document that already exists, checks nothing else already
holds that job, and handles what was built from the old version.

*"Set me up"* and *"let's redo the voice interview properly"* are this skill.
*"Stop using exclamation points"*, *"we charge $30 now"* and *"that's wrong"* are
`business-os:update-the-brain` — always, even mid-interview. Two skills claiming one
job is the same failure as two documents claiming one job.
...
**8 · Build and prove** — create everything (below), then run `business-os:draft-post`
...
9. **Three drafts** via `business-os:draft-post`.
...
`business-os:update-the-brain` — even mid-interview. Especially mid-interview.
