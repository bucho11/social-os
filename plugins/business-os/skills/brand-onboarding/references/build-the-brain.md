...
**Set `workspace_version` to the current shape** (see
`${CLAUDE_PLUGIN_ROOT}/migrations/_index.md` for what that is today) and open the
upgrade history with one `done` row. A workspace built today should never be
offered an upgrade it does not need — and a workspace with no version reads as
version 1, which would offer it every migration ever written.
...
Run `business-os:draft-post` on the first three Content Calendar ideas. Show her the
